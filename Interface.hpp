...
protected:
    Machine* _connected = nullptr;
    interface_t _interface;
    MAC _mac;
    Link* _linked_to = nullptr;
public:
    inline constexpr Basic_Interface(Machine& M, interface_t interface, MAC mac) : _connected(&M), _interface(interface), _mac(mac) {};
    inline constexpr Basic_Interface(Machine& M, interface_t interface, MAC mac, Link* l) : _connected(&M), _interface(interface), _mac(mac), _linked_to(l) {};
    ...
    inline Machine* get_machine() { return _connected; }
...
private:
    ...
    CIDR _cidr;
public:
    Interface(Machine& M, interface_t interface, MAC mac, IPv4 ip, CIDR cidr) : Basic_Interface(M, interface, mac), _ip(ip), _cidr(cidr) {};
    Interface(Machine& M, interface_t interface, MAC mac, Link* l, IPv4 ip, CIDR cidr) : Basic_Interface(M, interface, mac, l), _ip(ip), _cidr(cidr) {};
    inline IPv4 get_ip() { return _ip; }
    inline CIDR get_cidr() { return _cidr; }

    IP_Machine* get_machine();
